...
class FCNN(torch.nn.Module):
    def __init__(self, input_dim, output_dim, num_hidden_layers=4, hidden_dim=128, activation_fn=torch.nn.ReLU):
...
def generate_toy_data(n_features=6, num_samples=1000):
    X = np.random.rand(num_samples, n_features)
    # Linear relationship with noise
    y = 2 * np.sum(X, axis=1, keepdims=True) + 1 + 0.1 * np.random.randn(num_samples, 1)
    return X, y
...
def train_model(model, train_loader, val_loader, optimizer, loss_fn, num_epochs=500):
    ...
    for epoch in pbar:
        ...
        pbar.set_postfix({"Train Loss": f"{avg_train_loss:.4e}", "Val Loss": f"{val_loss:.4e}"})
...
def main():
    # Generate toy data
    X, y = generate_toy_data(n_features=6, num_samples=1000)
    ...
    model = FCNN(input_dim, output_dim, num_hidden_layers=2, hidden_dim=64, activation_fn=torch.nn.Tanh)
    ...
    # Train the model
    num_epochs = 1_000
    train_losses, val_losses = train_model(model, train_loader, val_loader, optimizer, loss_fn, num_epochs)
