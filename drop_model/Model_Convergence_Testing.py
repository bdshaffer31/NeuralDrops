...
def main():
    ...
    decades = 6
    volume = torch.zeros(decades)
    for idx in range(decades):
        Nt = 16*torch.power(10.0, idx-1)
        dt = 0.1*torch.power(0.1, idx-1)
        t_lin = torch.linspace(0, dt * Nt, Nt)

        h_history = utils.run_forward_euler_simulation(drop_model, h_0, t_lin, post_fn)
        
        volume[idx] = utils.calc_volume(params, drop_model.r, h_history[-1])
